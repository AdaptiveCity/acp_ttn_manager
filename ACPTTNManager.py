    ...
    def __get_migrate_device(self, device_settings, migrate_version):
        ...
        device['is'] = {
            "end_device": {
                ...
                "join_server_address": self.settings['template-v3']['join_server_address'],
                "network_server_address": self.settings['template-v3']['join_server_address'],
                "application_server_address": self.settings['template-v3']['join_server_address'],
        ...
        device['ns'] = {
            "end_device": {
                "frequency_plan_id": device_settings['frequency_plan_id'] if 'frequency_plan_id' in device_settings.keys() else self.settings['template-v3']['frequency_plan_id'],
                "supports_join": device_settings['supports_join'] if 'supports_join' in device_settings.keys() else True,
                "lorawan_version": device_settings['lorawan_version'] if 'lorawan_version' in device_settings.keys() else self.self.settings['template-v3']['frequency_plan_id'],                
                "lorawan_phy_version": device_settings['lorawan_phy_version'] if 'lorawan_phy_version' in device_settings.keys() else self.settings['template-v3']['frequency_plan_id']+"-a",
                "ids":{
                    "device_id": device_settings['ids']['device_id'] if migrate_version == 3 else device_settings['dev_id'],
                    "dev_eui": device_settings['ids']['dev_eui']  if migrate_version == 3 else device_settings['lorawan_device']['dev_eui'],
                    "join_eui": device_settings['ids']['join_eui'] if migrate_version == 3 else device_settings['lorawan_device']['app_eui']
